...
import { PRO_C20_LESSONS } from "../data/proC20/lessons";
import { SONGS } from "../data/songs";
import { useProgressStore } from "../store/useProgressStore";
import { LessonPlayer } from "../components/learn/LessonPlayer";
import { ProC20LessonPlayer } from "../components/proC20/learn/ProC20LessonPlayer";
import { SongPlayer } from "../components/SongPlayer";

type LearnInstrument = "mini" | "proC20";
...
export function LearnView() {
  ...
  const [instrument, setInstrument] = useState<LearnInstrument>("mini");
  const [openLesson, setOpenLesson] = useState<string | null>(null);
  const [openSong, setOpenSong] = useState<string | null>(null);

  if (openLesson) {
    const proLesson = PRO_C20_LESSONS.find((l) => l.id === openLesson);
    if (proLesson) {
      return <ProC20LessonPlayer lesson={proLesson} onExit={() => setOpenLesson(null)} />;
    }
  ...
  const isProDone = (id: string) => completedProC20Lessons.includes(id);
  const isProLocked = (i: number) =>
    !unlockAll && i > 0 && !isProDone(PRO_C20_LESSONS[i - 1].id);
  ...
  return (
    <div className="flex flex-col gap-3 p-3 pb-6">
      ...
      <div className="grid grid-cols-2 gap-2 rounded-full bg-white p-1 shadow-sm" role="tablist" aria-label="악기 선택">
        {[
          { id: "mini" as const, label: "Clarii mini" },
          { id: "proC20" as const, label: "PRO C20" },
        ].map((item) => {
          ...
          return (
            <button
              ...
              className={`rounded-full px-3 py-2 text-xs font-extrabold transition-colors ${
                active ? "bg-brand text-white" : "text-sub hover:bg-stage"
              }`}
            >
              {item.label}
            </button>
      ...
      <p className="text-xs font-semibold text-sub">
        {instrument === "mini"
          ...
          : `PRO C20 14-key Boehm 구조와 운지 모드 — 11개 레슨 (${completedProC20Lessons.length}/11 완료)`}
      </p>
      ...
      {instrument === "mini" ? (
        ...
        <ul className="flex flex-col gap-2">
          {PRO_C20_LESSONS.map((l, i) => {
            const done = isProDone(l.id);
            const locked = isProLocked(i);
            const savedPos = proC20LessonCardPos[l.id] ?? 0;
            return (
              <li key={l.id}>
                <button
                  type="button"
                  ...
                  className={`flex w-full items-center gap-3 rounded-card border-2 bg-white px-4 py-3 text-left shadow-sm transition-colors ${
                      ...
                      : locked
                        ? "border-transparent opacity-50"
                        : "border-transparent hover:border-[#19E6FF]/60"
                  }`}
                >
                  <span className="text-2xl" aria-hidden>
                    {locked ? "🔒" : l.emoji}
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block text-[10px] font-bold text-sub">{l.id}</span>
                    <span className="block truncate text-sm font-extrabold">{l.title}</span>
                    <span className="mt-0.5 block truncate text-[11px] font-semibold text-sub">{l.summary}</span>
                  </span>
                  {done ? (
                    <span className="rounded-full bg-success/10 px-2 py-1 text-xs font-extrabold text-success">
                      ✓ 완료
                    ...
                    <span className="rounded-full bg-[#19E6FF]/10 px-2 py-1 text-xs font-extrabold text-[#087D86]">
                      {savedPos + 1}/{l.cards.length}
                    </span>
                  ) : null}
                </button>
              </li>
